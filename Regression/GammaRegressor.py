...
def GammaReg(data, labels, nof,file , filename ):
    params = {
        'alpha': [0 , 0.001 , 0.005 , 0.01 , 0.05 , 0.1 , 0.5 ,0.9 ,1]
    }
    clf = GridSearchCV(GammaRegressor(), params, n_jobs=-1, cv=nof)
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    al = best_parameters['alpha']


    model = GammaRegressor( alpha=al)
    
    r = RegressorMetrics()
    r.set_predictorName("GammaRegressor")
    r.set_contin(False)
