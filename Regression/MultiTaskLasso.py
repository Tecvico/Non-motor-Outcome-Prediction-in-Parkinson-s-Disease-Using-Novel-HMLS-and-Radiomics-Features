...
def MultiTaskLassoReg(data, labels, nof ,file , filename):
    params = {
        'alpha': [0 , 0.001 , 0.005 , 0.01 , 0.05 , 0.1 , 0.5 ,0.9 ,1],
    }
    clf = GridSearchCV(MultiTaskLasso(), params, n_jobs=-1, cv=nof)
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    al = best_parameters['alpha']

    model = MultiTaskLasso( alpha=al )
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = RegressorMetrics()
    r.set_predictorName("MultiTaskLasso")
    r.set_contin(False)
