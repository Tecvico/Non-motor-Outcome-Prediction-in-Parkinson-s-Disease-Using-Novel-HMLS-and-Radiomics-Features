...
def HuberReg(data, labels, nof,file , filename ):
    params = {
        'alpha': [0 , 0.001 , 0.005 , 0.01 , 0.05 , 0.1 , 0.5 ,0.9 ,1],
        'epsilon': [1 , 1.1 , 1.35 , 1.5 ,1.7 ,2],
    }
    clf = GridSearchCV(HuberRegressor(), params, n_jobs=-1, cv=nof)
    clf.fit(data, labels)
    best_parameters = clf.best_params_
    al = best_parameters['alpha']
    ep = best_parameters['epsilon']

    model = HuberRegressor(alpha=al , epsilon = ep)
    
    r = RegressorMetrics()
    r.set_predictorName("HuberRegressor")
    r.set_contin(False)
    r.set_DatasetName(filename)
    bestparameters = 'alpha= ' +str(al) + ' epsilon= '+str(ep)
