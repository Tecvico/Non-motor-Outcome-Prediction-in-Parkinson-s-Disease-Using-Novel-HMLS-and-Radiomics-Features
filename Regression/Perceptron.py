...
def PerceptronReg(data, labels, nof ,file , filename):
    params = {
        'alpha': [0 , 0.001 , 0.005 , 0.01 , 0.05 , 0.1 , 0.5 ,0.9 ,1],
        'l1_ratio' : [0 , 0.1 , 0.5 , 0.9 ,1],
        'penalty' : ['l2','l1','elasticnet']
    }
    clf = GridSearchCV(Perceptron(), params, n_jobs=-1, cv=nof)
    clf.fit(data, labels)
    ...
    model = Perceptron( alpha=al ,l1_ratio=lr , penalty=pn)
    # cvs = cross_val_score(model, data, labels, cv=nof)
    # print(cvs)
    # print(cvs.mean() , "\t" , cvs.std())
    r = RegressorMetrics()
    r.set_predictorName("Perceptron")
    r.set_contin(False)
    r.set_DatasetName(filename)
    bestparameters = 'alpha= ' +str(al) + ' l1_ratio= '+str(lr) + ' penalty= '+str(pn)
